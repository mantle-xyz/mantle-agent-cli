...
export function ensureEndpointAllowed(endpoint) {
    let url;
    try {
        url = new URL(endpoint);
    }
    catch {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Invalid endpoint URL: ${endpoint}`, "Provide a valid absolute endpoint URL.", { endpoint });
    }
    ...
    if (url.protocol !== "https:" && !allowLoopbackHttp) {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Endpoint protocol not allowed: ${url.protocol}`, "Use https:// endpoints, or enable local http for localhost only.", { endpoint: url.toString() });
    }
    if (!allowLoopbackHttp && (isPrivateIpv4(host) || isBlockedIpv6(host))) {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Endpoint host is private or local and is not allowed: ${host}`, "Use a public endpoint host.", { endpoint: url.toString(), host });
    }
    const metadataHosts = new Set(["169.254.169.254", "metadata.google.internal"]);
    if (metadataHosts.has(host.toLowerCase())) {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Metadata endpoint is blocked: ${host}`, "Use a non-metadata endpoint.", { endpoint: url.toString(), host });
    }
    const allowlist = parseAllowlist();
    if (allowlist.length > 0 && !hostMatchesAllowlist(host, allowlist)) {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Endpoint host is not in allowlist: ${host}`, "Set MANTLE_ALLOWED_ENDPOINT_DOMAINS to include this host, or use an allowed endpoint.", { endpoint: url.toString(), host, allowlist });
    }
    return url;
}
/**
 ...
 */
async function rejectPrivateResolution(hostname) {
    // Skip resolution for direct IP inputs — already checked in ensureEndpointAllowed
    if (isIpv4(hostname) || isBlockedIpv6(hostname))
        return;
    let addresses = [];
    let addresses6 = [];
    ...
    if (resolve4Failed && resolve6Failed) {
        throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Cannot resolve hostname '${hostname}'. Unresolvable hosts are rejected for safety.`, "Use a hostname that resolves to a public IP address via DNS.", { hostname });
    }
    const allAddresses = [...addresses, ...addresses6];
    for (const addr of allAddresses) {
        if (isPrivateIpv4(addr) || isBlockedIpv6(addr) || addr === "0.0.0.0") {
            throw new MantleMcpError("ENDPOINT_NOT_ALLOWED", `Hostname '${hostname}' resolves to private/loopback address ${addr}.`, "Use a hostname that resolves to a public IP address.", { hostname, resolved_address: addr });
        }
    }
}
/**
 ...
 */
export async function ensureEndpointSafe(endpoint) {
 ...
 */
export function safeFetchOptions(options = {}) {
    return {
        ...options,
        redirect: "error"
    };
 ...
 */
export function ensureReadOnlySql(query) {
    const trimmed = query.trim();
    // Reject empty queries
    if (!trimmed) {
        throw new MantleMcpError("INDEXER_ERROR", "Empty SQL query.", "Submit a non-empty SELECT query.", { query });
    }
    ...
    if (/;[\s]*\S/.test(withoutStrings)) {
        throw new MantleMcpError("INDEXER_ERROR", "Multi-statement SQL queries are not allowed.", "Submit a single SELECT statement only.", { query });
    }
    ...
    if (!isSelect && !isWithSelect) {
        throw new MantleMcpError("INDEXER_ERROR", "Only SELECT queries are allowed.", "Submit a read-only SELECT or WITH ... SELECT query.", { query });
    }
    // Reject known dangerous constructs even inside SELECT-like queries
    const dangerousPatterns = /\b(insert|update|delete|drop|alter|create|truncate|grant|revoke|merge|call|exec|execute|copy|load|import|into\s+outfile|into\s+dumpfile)\b/i;
    if (dangerousPatterns.test(normalized)) {
        throw new MantleMcpError("INDEXER_ERROR", "SQL query contains disallowed mutation or procedure keywords.", "Submit a pure read-only SELECT query without mutation side-effects.", { query });
    }
}
