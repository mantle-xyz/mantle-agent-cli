...
export async function getBalance(args, deps) {
...
export async function getTokenBalances(args, deps) {
    const resolvedDeps = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const address = requireAddress(args.address, "address");
    ...
    const balances = await Promise.all(tokens.map(async (rawToken) => {
        ...
        try {
            ...
            return {
                ...
                balance_normalized: resolved.decimals == null ? null : formatUnits(balanceRaw, resolved.decimals),
                error: null
            };
        }
        catch (error) {
...
export async function getAllowances(args, deps) {
    const resolvedDeps = withDeps(deps);
    const { network } = normalizeNetwork(args);
    ...
    const client = resolvedDeps.getClient(network);
    const blockNumber = await client.getBlockNumber();
    const allowances = await Promise.all(pairs.map(async (pairRaw) => {
        const pair = pairRaw;
        ...
        try {
            ...
            const allowanceRaw = await resolvedDeps.readTokenAllowance(client, token.address, owner, spender);
            return {
                ...
                allowance_normalized: token.decimals == null ? null : formatUnits(allowanceRaw, token.decimals),
...
export const accountTools = {
    getBalance: {
        ...
        description: "Get native MNT balance for an address. Examples: address=0x458F293454fE0d67EC0655f3672301301DD51422 -> balance_mnt='1.5' before contract operations.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    getTokenBalances: {
        ...
        description: "Batch read ERC-20 token balances for an address. Examples: tokens=['USDC','WETH'] -> 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9 and 0xdEAddEaDdeadDEadDEADDEAddEADDEAddead1111.",
        inputSchema: {
            type: "object",
            properties: {
                address: { type: "string", description: "Wallet address." },
    ...
    getAllowances: {
        ...
        description: "Batch read ERC-20 allowances for token/spender pairs. Examples: pair {token:'USDC', spender:'0x319B69888b0d11cEC22caA5034e25FfFBDc88421'} for Agni Router.",
        inputSchema: {
            type: "object",
            properties: {
