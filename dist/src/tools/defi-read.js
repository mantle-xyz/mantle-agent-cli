...
function resolveDefiLlamaChain(network) {
    if (network === "mainnet") {
        return "mantle";
    }
    return null;
}
function providerToDexId(provider) {
    if (provider === "fluxion")
        return "0xf883162ed9c7e8ef604214c964c678e40c9b737c";
...
function buildConfidence(params) {
    ...
    const score = clampScore(coverage * 0.5 + freshness * 0.3 + sourceCount * 0.2 - conflictPenalty);
    return {
...
function unpackTokenPriceLookup(payload) {
...
function normalizeProtocolTvlValue(value) {
    if (value == null) {
        return null;
    }
    if (typeof value === "number") {
...
function latestTvlPointFromSeries(series) {
    ...
    for (const item of series) {
        ...
        if (normalized.updated_at_unix != null &&
...
function extractLatestDefiLlamaProtocolTvl(payload, network) {
    ...
    const chainTvls = record.chainTvls;
    const chainEntry = chainTvls && typeof chainTvls === "object"
        ? chainTvls.Mantle
        : undefined;
    const chainPoint = latestTvlPointFromSeries(chainEntry?.tvl) ??
...
async function fetchDefiLlamaProtocolTvl(protocol, network) {
...
function normalizeProtocolInput(input) {
    ...
    throw new MantleMcpError("UNSUPPORTED_PROTOCOL", `Unsupported protocol for TVL: ${input}`, "Use protocol=agni, protocol=merchant_moe, or protocol=all.", { protocol: input });
}
...
async function fetchDexScreenerTokenPairs(network, tokenAddress) {
    const chainId = resolveDexScreenerChain(network);
    if (!chainId) {
        return [];
    }
    const payload = await fetchJsonSafe(`${DEXSCREENER_API_BASE}/token-pairs/v1/${chainId}/${tokenAddress}`);
    return Array.isArray(payload) ? payload : [];
}
async function fetchDexScreenerPairByAddress(network, poolAddress) {
    const chainId = resolveDexScreenerChain(network);
    if (!chainId) {
        return null;
    }
    ...
    return (pairs.find((pair) => addressesEqual(pair.pairAddress, poolAddress)) ??
        pairs[0] ??
        null);
}
function pickBestDexPairForRoute(pairs, provider, tokenInAddress, tokenOutAddress) {
    ...
    const candidates = pairs.filter((pair) => {
        ...
        return ((addressesEqual(base, tokenInAddress) && addressesEqual(quote, tokenOutAddress)) ||
...
async function fetchDexScreenerTokenPriceUsd(network, tokenAddress) {
    const chainId = resolveDexScreenerChain(network);
    if (!chainId) {
        return null;
    }
    const payload = await fetchJsonSafe(`${DEXSCREENER_API_BASE}/tokens/v1/${chainId}/${tokenAddress}`);
    if (!Array.isArray(payload) || payload.length === 0) {
        return null;
    }
    const pairs = payload;
    const best = pairs.find((pair) => addressesEqual(pair.baseToken?.address, tokenAddress)) ??
        pairs[0];
    return asFiniteNumber(best?.priceUsd);
}
async function fetchDefiLlamaTokenPrices(network, tokenAddresses) {
    ...
    const coins = payload && typeof payload === "object" && payload.coins && typeof payload.coins === "object"
        ? payload.coins
        : {};
    const out = {};
...
const defaultSwapDeps = {
    ...
    quoteProvider: async ({ provider, tokenIn, tokenOut, amountInRaw, network, feeTier }) => {
        ...
        const pair = pickBestDexPairForRoute(pairs, provider, tokenIn.address, tokenOut.address);
        if (!pair) {
            return null;
        }
...
const defaultPoolDeps = {
    readPool: async ({ poolAddress, provider, network }) => {
        ...
        if (!isAddress(token0Address, { strict: false }) ||
    ...
    getTokenPrices: async ({ network, tokenAddresses }) => {
        ...
        const out = {};
        const sourceTrace = [];
        const dexPrices = await Promise.all(normalized.map((address) => fetchDexScreenerTokenPriceUsd(network, address)));
        const missing = [];
        ...
        sourceTrace.push({
            ...
            reason: missing.length === normalized.length
...
const defaultPoolOpportunityDeps = {
    resolveTokenInput: (token, network) => resolveTokenInputFromRegistry(token, network ?? "mainnet"),
...
function toUsdValue(supplyRaw, decimals, assetPriceInBase, baseCurrencyUnit) {
...
async function loadAaveV3Markets(network) {
    if (network !== "mainnet") {
        return [];
    }
    ...
    if (!poolAddressesProviderInput ||
        !dataProviderInput ||
        !isAddress(poolAddressesProviderInput, { strict: false }) ||
        !isAddress(dataProviderInput, { strict: false })) {
        throw new MantleMcpError("LENDING_DATA_UNAVAILABLE", "Aave V3 provider addresses are missing or invalid.", "Verify Mantle protocol registry for aave_v3 contract addresses.", { network, protocol: "aave_v3" });
    }
    ...
    const poolAddress = poolInput && isAddress(poolInput, { strict: false })
    ...
    if (!isAddress(oracleAddress, { strict: false })) {
        throw new MantleMcpError("LENDING_DATA_UNAVAILABLE", "Aave oracle address is invalid.", "Retry later or verify pool addresses provider configuration.", { network, protocol: "aave_v3", oracle_address: oracleAddress });
    }
    ...
    const markets = await Promise.all(reserves.map(async (reserve) => {
        ...
        const [reserveData, reserveConfig, assetPrice, rawConfig] = await Promise.all(rpcCalls);
        ...
        let borrowableInIsolation = null;
        let debtCeilingUsd = null;
    ...
    return markets.filter((item) => item !== null);
}
const defaultLendingDeps = {
    marketProvider: async ({ protocol, network }) => {
        ...
        try {
            ...
            throw new MantleMcpError("LENDING_DATA_UNAVAILABLE", error instanceof Error ? error.message : String(error), "Check RPC health and Aave provider/oracle reachability, then retry.", { network, protocol: "aave_v3", retryable: true });
        }
    },
...
function withSwapDeps(overrides) {
    return { ...defaultSwapDeps, ...overrides };
}
function withPoolDeps(overrides) {
    return { ...defaultPoolDeps, ...overrides };
}
function withPoolOpportunityDeps(overrides) {
    return { ...defaultPoolOpportunityDeps, ...overrides };
}
function withLendingDeps(overrides) {
    return { ...defaultLendingDeps, ...overrides };
}
function withProtocolTvlDeps(overrides) {
    return { ...defaultProtocolTvlDeps, ...overrides };
}
function resolveRouterAddress(provider, network) {
    const entry = MANTLE_PROTOCOLS[network][provider];
    if (!entry || entry.status !== "enabled") {
        throw new MantleMcpError("UNSUPPORTED_PROTOCOL", `${provider} is not enabled on ${network}.`, "Use an enabled protocol or switch network.", { provider, network });
    }
    const router = provider === "merchant_moe"
    ...
    if (!router || !isAddress(router, { strict: false })) {
        throw new MantleMcpError("UNSUPPORTED_PROTOCOL", `${provider} router is not configured for ${network}.`, "Check protocol registry configuration.", { provider, network });
    }
    return getAddress(router);
}
function parseRawAmount(value, field, details) {
    try {
        return BigInt(value);
    }
    catch {
        throw new MantleMcpError("INVALID_INPUT", `${field} must be a base-10 integer string.`, "Check upstream data source and retry.", details);
    }
}
export async function getSwapQuote(args, deps) {
    ...
    const providerInput = typeof args.provider === "string" ? args.provider : "best";
    const feeTier = typeof args.fee_tier === "number" ? args.fee_tier : null;
    if (!tokenInInput || !tokenOutInput || !amountInInput) {
        throw new MantleMcpError("INVALID_INPUT", "token_in, token_out, and amount_in are required.", "Provide token_in, token_out, and amount_in values.", { token_in: tokenInInput || null, token_out: tokenOutInput || null, amount_in: amountInInput || null });
    }
    ...
    if (providerInput === "agni" || providerInput === "merchant_moe" || providerInput === "fluxion" || providerInput === "best") {
        ...
        throw new MantleMcpError("INVALID_INPUT", `Unsupported provider: ${providerInput}`, "Use provider=agni, provider=fluxion, provider=merchant_moe, or provider=best.", { provider: providerInput });
    }
    ...
    if (tokenIn.decimals == null || tokenOut.decimals == null || tokenIn.address === "native" || tokenOut.address === "native") {
        throw new MantleMcpError("TOKEN_NOT_FOUND", "Swap quote requires ERC-20 tokens with known decimals.", "Use ERC-20 token symbols/addresses available in mantle://registry/tokens.", { token_in: tokenInInput, token_out: tokenOutInput });
    }
    if (tokenIn.address.toLowerCase() === tokenOut.address.toLowerCase()) {
        throw new MantleMcpError("INVALID_INPUT", "token_in and token_out must be different.", "Choose two different ERC-20 tokens for swap quote.", {
    ...
    try {
        ...
        throw new MantleMcpError("INVALID_INPUT", "amount_in is not a valid decimal amount.", "Provide a positive decimal amount_in with token precision.", { amount_in: amountInInput, token_decimals: tokenIn.decimals });
    }
    if (amountInRaw <= 0n) {
        throw new MantleMcpError("INVALID_INPUT", "amount_in must be greater than zero.", "Provide a positive decimal amount_in.", { amount_in: amountInInput });
    }
    const warnings = [];
    const sourceTrace = [];
    ...
    if (providerSelection === "best") {
        ...
        if (candidates.length === 0) {
            throw new MantleMcpError("NO_ROUTE", `No route found for ${tokenIn.symbol} -> ${tokenOut.symbol}.`, "Try another provider or pair.", {
                token_in: tokenIn.address,
                token_out: tokenOut.address,
                provider: "best",
                network
...
function deriveLiquidityUsdFromReserves(reserve0Raw, reserve1Raw, decimals0, decimals1, price0Usd, price1Usd) {
...
export async function getPoolOpportunities(args, deps) {
    ...
    if (!tokenAInput || !tokenBInput) {
        throw new MantleMcpError("INVALID_INPUT", "token_a and token_b are required.", "Provide token_a and token_b as symbols or token addresses.", { token_a: tokenAInput || null, token_b: tokenBInput || null });
    }
    const providerSelection = providerInput === "agni" || providerInput === "merchant_moe" || providerInput === "fluxion" || providerInput === "all"
        ? providerInput
        : "all";
    ...
    const normalizeForDex = async (token) => {
        if (token.address === "native") {
            const wrapped = await resolvedDeps.resolveTokenInput("WMNT", network);
            if (wrapped.address === "native") {
                throw new MantleMcpError("TOKEN_NOT_FOUND", "WMNT is required for native MNT pool discovery.", "Ensure WMNT is available in mantle://registry/tokens.", { network });
            }
    ...
    if (addressesEqual(tokenAAddress, tokenBAddress)) {
        throw new MantleMcpError("INVALID_INPUT", "token_a and token_b must be different tokens.", "Select two distinct tokens for pool discovery.", { token_a: tokenAInput, token_b: tokenBInput });
    }
    ...
    const candidates = pairs
        ...
        const base = pair.baseToken?.address ?? null;
        const quote = pair.quoteToken?.address ?? null;
        return ((addressesEqual(base, tokenAAddress) && addressesEqual(quote, tokenBAddress)) ||
            (addressesEqual(base, tokenBAddress) && addressesEqual(quote, tokenAAddress)));
    })
        .map((pair) => {
        const provider = dexIdToProvider(pair.dexId);
        const poolAddress = pair.pairAddress && isAddress(pair.pairAddress, { strict: false })
    ...
    if (candidates.length > 0) {
        ...
        throw new MantleMcpError("NO_ROUTE", "No matching liquidity pool candidates found.", "Try a different token pair or provider filter.", {
    ...
    const ranked = candidates
        .map((item) => {
        const liquidityScore = maxLiquidity > 0 && item.liquidity_usd != null ? item.liquidity_usd / maxLiquidity : 0;
        const volumeScore = maxVolume > 0 && item.volume_24h_usd != null ? item.volume_24h_usd / maxVolume : 0;
        ...
        .slice(0, maxResults);
    const coverage = ranked.length === 0
        ? 0
        : ranked
...
export async function getPoolLiquidity(args, deps) {
    ...
    const provider = providerInput === "merchant_moe" ? "merchant_moe" : providerInput === "fluxion" ? "fluxion" : "agni";
    if (!poolAddressInput || !isAddress(poolAddressInput, { strict: false })) {
        throw new MantleMcpError("INVALID_ADDRESS", "pool_address must be a valid address.", "Provide a checksummed pool address.", { pool_address: poolAddressInput || null });
    }
    ...
    if (!data) {
        ...
        throw new MantleMcpError("POOL_NOT_FOUND", `Pool not found: ${poolAddress}`, "Verify the pool address and provider.", { pool_address: poolAddress, provider, network, source_trace: sourceTrace });
    }
    ...
    const reserve0Decimal = data.token_0.decimals == null ? null : formatUnits(reserve0Raw, data.token_0.decimals);
    const reserve1Decimal = data.token_1.decimals == null ? null : formatUnits(reserve1Raw, data.token_1.decimals);
    const warnings = [];
    let hasConflict = false;
    let totalLiquidityUsdRange = null;
    let totalLiquidityUsd = typeof data.total_liquidity_usd === "number" ? data.total_liquidity_usd : null;
    let derivedLiquidityUsd = null;
    if (data.token_0.decimals != null &&
        ...
        if (typeof token0Price === "number" && typeof token1Price === "number") {
            const derived = deriveLiquidityUsdFromReserves(reserve0Raw, reserve1Raw, data.token_0.decimals, data.token_1.decimals, token0Price, token1Price);
...
export async function getLendingMarkets(args, deps) {
    ...
    const protocol = protocolInput === "aave" ? "aave_v3" : protocolInput;
    if (!["all", "aave_v3"].includes(protocol)) {
        throw new MantleMcpError("UNSUPPORTED_PROTOCOL", `Unsupported lending protocol: ${protocolInput}`, "Use protocol=aave_v3, aave, or all.", { protocol: protocolInput });
    }
    const sourceTrace = [];
    let tierUsed = 1;
    ...
    if (allMarkets.length === 0 && network === "mainnet" && protocol === "aave_v3") {
        throw new MantleMcpError("LENDING_DATA_UNAVAILABLE", "No Aave V3 market data could be retrieved for Mantle mainnet.", "Retry after checking RPC health, or run mantle_checkRpcHealth first.", { protocol: "aave_v3", network, source_trace: sourceTrace });
    }
    const filteredMarkets = asset
        ? allMarkets.filter((market) => market.asset.toLowerCase() === asset.toLowerCase() ||
...
async function resolveSingleProtocolTvl(params) {
    ...
    const warnings = [];
    const values = [];
    const defillamaValue = normalizeProtocolTvlValue(await deps.protocolTvlProvider({ protocol, network }));
    if (defillamaValue) {
        values.push({
    ...
    const subgraphValue = normalizeProtocolTvlValue(await deps.protocolTvlFromSubgraph({ protocol, network }));
    if (subgraphValue) {
        values.push({
            source: "subgraph",
            tier: 2,
    ...
    const indexerValue = normalizeProtocolTvlValue(await deps.protocolTvlFromIndexer({ protocol, network }));
    if (indexerValue) {
        values.push({
            source: "indexer_sql",
            tier: 3,
    ...
    return {
        ...
        updated_at_utc: primary?.updated_at_unix != null
...
export async function getProtocolTvl(args, deps) {
    const resolvedDeps = withProtocolTvlDeps(deps);
    const { network } = normalizeNetwork(args);
    const protocolInput = typeof args.protocol === "string" ? args.protocol : "all";
    const protocol = normalizeProtocolInput(protocolInput);
    const targets = protocol === "all" ? ["agni", "merchant_moe"] : [protocol];
    const breakdown = await Promise.all(targets.map((target) => resolveSingleProtocolTvl({
        protocol: target,
        network,
        deps: resolvedDeps
    })));
    if (protocol !== "all" && breakdown[0]?.tvl_usd == null) {
        throw new MantleMcpError("TVL_DATA_UNAVAILABLE", `No protocol TVL data available for ${protocol}.`, "Check DefiLlama/subgraph/indexer reachability and retry.", {
            protocol,
            network,
...
export const defiReadTools = {
    getSwapQuote: {
        name: "mantle_getSwapQuote",
        description: "Read swap quotes for Agni and Merchant Moe routes. Examples: WMNT 0x78c1b0C915c4FAA5FffA6CAbf0219DA63d7f4cb8 to USDC 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9 via Agni router 0x319B69888b0d11cEC22caA5034e25FfFBDc88421.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    getPoolLiquidity: {
        ...
        description: "Read pool reserves and liquidity metadata. Examples: inspect a Mantle DEX pool for USDC 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9 / USDT 0x201EBa5CC46D216Ce6DC03F6a759e8E766e956aE liquidity.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    getPoolOpportunities: {
        ...
        description: "Scan and rank candidate pools for a token pair on Mantle DEXes. Examples: token_a='MNT' (native mapped to WMNT 0x78c1b0C915c4FAA5FffA6CAbf0219DA63d7f4cb8), token_b='mETH' 0xcDA86A272531e8640cD7F1a92c01839911B90bb0.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    getProtocolTvl: {
        ...
        description: "Read protocol-level TVL for Mantle DeFi protocols using layered sources. Examples: protocol=agni for Agni Finance TVL, protocol=merchant_moe for Merchant Moe TVL, protocol=all for combined view.",
        inputSchema: {
            type: "object",
            properties: {
                protocol: {
    ...
    getLendingMarkets: {
        ...
        description: "Read Aave V3 lending market metrics on Mantle, including Isolation Mode data.\n\n" +
