...
import { MANTLE_PROTOCOLS, isWhitelistedContract, whitelistLabel } from "../config/protocols.js";
...
async function fetchTokenPriceUsd(network, tokenAddress) {
    if (network !== "mainnet")
        return null;
    // Try DexScreener first
    const dsPayload = await fetchJsonSafe(`${DEXSCREENER_API_BASE}/tokens/v1/mantle/${tokenAddress}`);
    if (Array.isArray(dsPayload) && dsPayload.length > 0) {
        const best = dsPayload.find((p) => p.baseToken?.address?.toLowerCase() === tokenAddress.toLowerCase()) ?? dsPayload[0];
        const price = typeof best?.priceUsd === "string" ? Number(best.priceUsd) : null;
        if (price != null && Number.isFinite(price) && price > 0)
            return price;
    }
    // Fallback to DefiLlama
    const llamaPayload = await fetchJsonSafe(`${DEFILLAMA_PRICES_API_BASE}/mantle:${tokenAddress.toLowerCase()}`);
...
import { findReserveBySymbol, findReserveByUnderlying, aaveReserveSymbols, isolationModeSymbols, isolationBorrowableSymbols, AAVE_V3_MANTLE_RESERVES } from "../config/aave-reserves.js";
import { findPair, findPairByAddress, listPairs, listAllPairs } from "../config/dex-pairs.js";
// ABIs
import { WMNT_ABI } from "../lib/abis/wmnt.js";
import { V3_SWAP_ROUTER_ABI, V3_POSITION_MANAGER_ABI } from "../lib/abis/uniswap-v3.js";
import { LB_ROUTER_ABI } from "../lib/abis/merchant-moe-lb.js";
 ...
 */
const BRIDGE_TOKENS = ["WMNT", "USDC", "USDT0", "USDe", "WETH"];
 ...
 */
function requireAaveReserve(symbolOrAddress) {
    const reserve = findReserveBySymbol(symbolOrAddress) ??
        findReserveByUnderlying(symbolOrAddress);
    if (!reserve) {
        throw new MantleMcpError("UNSUPPORTED_AAVE_ASSET", `'${symbolOrAddress}' is not a supported Aave V3 reserve on Mantle.`, `Supported assets: ${aaveReserveSymbols().join(", ")}.`, { asset: symbolOrAddress, supported: aaveReserveSymbols() });
    }
    return reserve;
}
const defaultDeps = {
...
async function resolveToken(d, input, network) {
    const resolved = await d.resolveTokenInput(input, network);
    if (resolved.decimals == null) {
        throw new MantleMcpError("TOKEN_NOT_FOUND", `Cannot determine decimals for token '${input}'. Provide a known symbol or token address from the Mantle token list.`, "Use a well-known token symbol (WMNT, USDC, USDT, USDe, mETH) or a verified address.", { token: input });
    }
    return {
...
function requirePositiveAmount(input, fieldName, decimals) {
    ...
    try {
        ...
        throw new MantleMcpError("INVALID_INPUT", `${fieldName} is not a valid decimal number.`, "Provide a positive decimal amount (e.g. '1.5').", { field: fieldName, value: str });
    }
    if (parsed <= 0n) {
        throw new MantleMcpError("INVALID_INPUT", `${fieldName} must be greater than zero.`, "Provide a positive amount.", { field: fieldName, value: str });
    }
    return parsed;
}
function requireProvider(input) {
    const str = typeof input === "string" ? input.toLowerCase().trim() : "";
    const valid = ["agni", "fluxion", "merchant_moe"];
    if (!valid.includes(str)) {
        throw new MantleMcpError("UNSUPPORTED_PROTOCOL", `Unsupported provider: ${String(input)}`, "Use one of: agni, fluxion, merchant_moe.", { provider: input ?? null });
    }
    return str;
}
...
// =========================================================================
export async function buildApprove(args, deps) {
    ...
    if (!isWhitelistedContract(spender, network)) {
        throw new MantleMcpError("SPENDER_NOT_WHITELISTED", `Spender ${spender} is not a whitelisted contract.`, "Only whitelisted protocol contracts can be approved as spenders.", { spender, network });
    }
    ...
    const amountDecimal = amountRaw === MAX_UINT256
        ? "unlimited"
    ...
    if (amountRaw === MAX_UINT256) {
        warnings.push("Unlimited approval granted. Consider using exact amounts for better security.");
    }
    return {
        intent: "approve",
...
// =========================================================================
export async function buildWrapMnt(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
...
// =========================================================================
export async function buildUnwrapMnt(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const amountRaw = requirePositiveAmount(args.amount, "amount", 18);
    const amountDecimal = formatUnits(amountRaw, 18);
 ...
 */
function findV3Route(provider, tokenIn, tokenOut, network) {
    for (const bridge of BRIDGE_TOKENS) {
        // Skip if bridge is same as in or out
        if (bridge.toLowerCase() === tokenIn.symbol.toLowerCase())
            continue;
        if (bridge.toLowerCase() === tokenOut.symbol.toLowerCase())
            continue;
        const legA = findPair(provider, tokenIn.symbol, bridge, network) ??
            findPairByAddress(provider, tokenIn.address, "", network); // fallback not useful here
        const legB = findPair(provider, bridge, tokenOut.symbol, network);
        if (legA && legB && legA.provider !== "merchant_moe" && legB.provider !== "merchant_moe") {
            // Need to resolve the bridge token address from the pair
            const bridgeAddress = legA.tokenA.toLowerCase() === tokenIn.symbol.toLowerCase()
                ? legA.tokenBAddress
                : legA.tokenAAddress;
            return {
                tokens: [
                ...
                fees: [legA.feeTier, legB.feeTier]
            };
        }
    }
    return null;
 ...
 */
function findMoeRoute(tokenIn, tokenOut, network) {
    for (const bridge of BRIDGE_TOKENS) {
        if (bridge.toLowerCase() === tokenIn.symbol.toLowerCase())
            continue;
        if (bridge.toLowerCase() === tokenOut.symbol.toLowerCase())
        ...
        if (legA && legB && legA.provider === "merchant_moe" && legB.provider === "merchant_moe") {
            ...
            return {
                ...
                binSteps: [legA.binStep, legB.binStep],
                routerVersions: [
                    legA.routerVersion ?? 0,
                    legB.routerVersion ?? 0
                ]
            };
        }
    }
...
export async function buildSwap(args, deps) {
    ...
    const amountInRaw = requirePositiveAmount(args.amount_in, "amount_in", tokenIn.decimals);
    const slippageBps = typeof args.slippage_bps === "number" ? args.slippage_bps : 50; // default 0.5%
    const recipient = requireAddress(args.recipient, "recipient");
    // Auto-resolve pair params from known pairs registry
    const knownPair = findPair(provider, tokenIn.symbol, tokenOut.symbol, network) ??
    ...
    if (typeof args.amount_out_min === "string" && args.amount_out_min !== "0" && args.amount_out_min.trim().length > 0) {
        ...
        throw new MantleMcpError("MISSING_SLIPPAGE_PROTECTION", "amount_out_min is required to protect against slippage and sandwich attacks.", "Call mantle_getSwapQuote first to get a quote, then pass amount_out_min (or minimum_out_raw from the quote). " +
            "Set allow_zero_min=true only if you understand the risks of unprotected swaps.", { token_in: tokenIn.symbol, token_out: tokenOut.symbol, amount_in: args.amount_in });
    }
    ...
    if (provider === "agni" || provider === "fluxion") {
        ...
        const available = listPairs(provider).map((p) => `${p.tokenA}/${p.tokenB}`);
        throw new MantleMcpError("UNKNOWN_PAIR", `No known fee_tier or multi-hop route for ${tokenIn.symbol}/${tokenOut.symbol} on ${provider}. Provide fee_tier explicitly.`, `Known pairs on ${provider}: ${available.join(", ") || "none"}. Common fee tiers: 500 (0.05%), 3000 (0.3%), 10000 (1%).`, { provider, tokenIn: tokenIn.symbol, tokenOut: tokenOut.symbol });
    }
    // merchant_moe — resolve bin_step: caller > known pair > multi-hop > error
    let binStep;
    let routerVersion = 0; // default V1 — works for all Moe pools
    if (typeof args.bin_step === "number") {
        binStep = args.bin_step;
    }
    else if (knownPair && knownPair.provider === "merchant_moe") {
        binStep = knownPair.binStep;
    }
    if (binStep !== undefined) {
        ...
        if (typeof args.router_version === "number") {
            ...
            routerVersion = knownPair.routerVersion ?? 0;
        }
        return buildMoeSwap({
            tokenIn,
            tokenOut,
    ...
    const available = listPairs("merchant_moe").map((p) => `${p.tokenA}/${p.tokenB}`);
    throw new MantleMcpError("UNKNOWN_PAIR", `No known bin_step or multi-hop route for ${tokenIn.symbol}/${tokenOut.symbol} on Merchant Moe. Provide bin_step explicitly.`, `Known pairs on Merchant Moe: ${available.join(", ")}. Common bin steps: 1 (stablecoins), 5 (LSTs), 20 (volatile).`, { provider, tokenIn: tokenIn.symbol, tokenOut: tokenOut.symbol });
}
function buildV3Swap(params) {
    const { provider, tokenIn, tokenOut, amountInRaw, amountInDecimal, amountOutMin, slippageBps, recipient, deadline, network, feeTier, now } = params;
    ...
    const warnings = [];
    if (amountOutMin === 0n) {
        warnings.push(`WARNING: amountOutMinimum is 0 — this swap has NO slippage protection and is vulnerable to sandwich attacks. Call mantle_getSwapQuote first and pass amount_out_min.`);
    }
...
function buildMoeSwap(params) {
    const { tokenIn, tokenOut, amountInRaw, amountInDecimal, amountOutMin, slippageBps, recipient, deadline, network, binStep, routerVersion, now } = params;
    ...
    const warnings = [];
    if (amountOutMin === 0n) {
        warnings.push("WARNING: amountOutMin is 0 — this swap has NO slippage protection. Call mantle_getSwapQuote first and pass amount_out_min.");
    }
    return {
        intent: "swap",
...
function buildV3MultihopSwap(params) {
    const { provider, route, tokenIn, tokenOut, amountInRaw, amountInDecimal, amountOutMin, recipient, deadline, network, now } = params;
    const routerAddress = getContractAddress(provider, "swap_router", network);
    ...
    const warnings = [];
    if (amountOutMin === 0n) {
        warnings.push("WARNING: amountOutMinimum is 0. Multi-hop swaps have higher slippage risk — call mantle_getSwapQuote first and pass amount_out_min.");
    }
...
function buildMoeMultihopSwap(params) {
    const { route, tokenIn, tokenOut, amountInRaw, amountInDecimal, amountOutMin, recipient, deadline, network, now } = params;
    const routerAddress = getContractAddress("merchant_moe", "lb_router_v2_2", network);
    const hops = route.tokens.map((t) => t.symbol).join(" → ");
    const path = {
        pairBinSteps: route.binSteps.map(BigInt),
        versions: route.routerVersions,
        tokenPath: route.tokens.map((t) => t.address)
    };
    const data = encodeFunctionData({
        abi: LB_ROUTER_ABI,
        functionName: "swapExactTokensForTokens",
    ...
    const warnings = [];
    if (amountOutMin === 0n) {
        warnings.push("amountOutMin is 0. Multi-hop swaps have higher slippage risk — call mantle_getSwapQuote first.");
    }
    warnings.push(`Multi-hop route: ${hops} (binSteps: ${route.binSteps.join(" → ")}, versions: ${route.routerVersions.join(" → ")})`);
    return {
        intent: "swap_multihop",
...
// =========================================================================
export async function buildAddLiquidity(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const provider = requireProvider(args.provider);
    ...
    if (args.amount_usd != null && args.amount_usd !== "") {
        const usdAmount = typeof args.amount_usd === "number"
            ? args.amount_usd
            : typeof args.amount_usd === "string"
                ? Number(args.amount_usd)
                : NaN;
        if (!Number.isFinite(usdAmount) || usdAmount <= 0) {
            throw new MantleMcpError("INVALID_INPUT", "amount_usd must be a positive number.", "Provide amount_usd as a positive decimal (e.g. 1000).", { amount_usd: args.amount_usd });
        }
        ...
        if (priceA == null || priceB == null) {
            throw new MantleMcpError("PRICE_UNAVAILABLE", `Cannot fetch USD price for ${priceA == null ? tokenA.symbol : tokenB.symbol}. USD amount mode requires price data.`, "Use amount_a and amount_b instead, or retry later.", {
        ...
        if (provider === "agni" || provider === "fluxion") {
            ...
            if (!isFullRange) {
                ...
                try {
                    ...
                    const [t0, t1, p0, p1] = tokenA.address.toLowerCase() < tokenB.address.toLowerCase()
                    ...
                    const poolAddr = await client.readContract({
                        ...
                        abi: [{ type: "function", name: "getPool", stateMutability: "view", inputs: [{ name: "", type: "address" }, { name: "", type: "address" }, { name: "", type: "uint24" }], outputs: [{ name: "", type: "address" }] }],
                        functionName: "getPool",
                        args: [t0.address, t1.address, feeTier]
                    });
                    if (poolAddr && poolAddr !== "0x0000000000000000000000000000000000000000") {
                        const slot0 = await client.readContract({
                            address: poolAddr,
                            abi: [{ type: "function", name: "slot0", stateMutability: "view", inputs: [], outputs: [{ name: "sqrtPriceX96", type: "uint160" }, { name: "tick", type: "int24" }, { name: "", type: "uint16" }, { name: "", type: "uint16" }, { name: "", type: "uint16" }, { name: "", type: "uint8" }, { name: "", type: "bool" }] }],
                            functionName: "slot0"
                        });
        ...
        warnings.push(`USD mode: $${usdAmount} split ${(ratioA * 100).toFixed(1)}/${((1 - ratioA) * 100).toFixed(1)} [${ratioMethod}] → ` +
        ...
        if ((args.amount_a == null || args.amount_a === "") &&
            (args.amount_b == null || args.amount_b === "")) {
            throw new MantleMcpError("INVALID_INPUT", "Either (amount_a + amount_b) or amount_usd is required.", "Provide token amounts directly with amount_a and amount_b, or use amount_usd for automatic USD-based sizing.", { amount_a: args.amount_a ?? null, amount_b: args.amount_b ?? null, amount_usd: args.amount_usd ?? null });
        }
        amountARaw = requirePositiveAmount(args.amount_a, "amount_a", tokenA.decimals);
        amountBRaw = requirePositiveAmount(args.amount_b, "amount_b", tokenB.decimals);
    }
    ...
    const result = buildMoeAddLiquidity({
        ...
        activeIdDesired: typeof args.active_id === "number" ? args.active_id : 8388608,
...
function buildV3AddLiquidity(params) {
    const { provider, tokenA, tokenB, amountARaw, amountBRaw, amountADecimal, amountBDecimal, slippageBps, recipient, deadline, network, feeTier, tickLower, tickUpper, now } = params;
    const positionManager = getContractAddress(provider, "position_manager", network);
    // Sort tokens (V3 requires token0 < token1)
    const [token0, token1, amount0Desired, amount1Desired, amt0Label, amt1Label] = tokenA.address.toLowerCase() < tokenB.address.toLowerCase()
    ...
    if (tickLower === -887220 && tickUpper === 887220) {
        warnings.push("Using full-range tick bounds (MIN_TICK to MAX_TICK). Consider narrower range for concentrated liquidity.");
    }
    return {
...
function buildMoeAddLiquidity(params) {
    const { tokenA, tokenB, amountARaw, amountBRaw, amountADecimal, amountBDecimal, slippageBps, recipient, deadline, network, binStep, activeIdDesired, idSlippage, deltaIds, distributionX, distributionY, now } = params;
    const routerAddress = getContractAddress("merchant_moe", "lb_router_v2_2", network);
    const amountAMin = (amountARaw * BigInt(10000 - slippageBps)) / 10000n;
    const amountBMin = (amountBRaw * BigInt(10000 - slippageBps)) / 10000n;
    const liquidityParameters = {
        tokenX: tokenA.address,
        tokenY: tokenB.address,
...
// =========================================================================
export async function buildRemoveLiquidity(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const provider = requireProvider(args.provider);
    const recipient = requireAddress(args.recipient, "recipient");
    const deadline = d.deadline();
    if (provider === "agni" || provider === "fluxion") {
        const tokenIdInput = typeof args.token_id === "number"
        ...
        if (args.percentage != null) {
            const pct = typeof args.percentage === "number"
                ? args.percentage
                : typeof args.percentage === "string"
                    ? Number(args.percentage)
                    : NaN;
            if (!Number.isFinite(pct) || pct <= 0 || pct > 100) {
                throw new MantleMcpError("INVALID_INPUT", "percentage must be between 0 (exclusive) and 100 (inclusive).", "Provide percentage as a number 1-100 (e.g. 50 for half, 100 for full removal).", { percentage: args.percentage });
            }
            ...
            const positionData = positionResult;
            const totalLiquidity = positionData[7];
            if (totalLiquidity === 0n) {
                throw new MantleMcpError("INVALID_INPUT", "Position has zero liquidity — nothing to remove.", "Check position status with mantle_getV3Positions first.", { token_id: tokenIdInput.toString(), provider });
            }
            liquidityToRemove = (totalLiquidity * BigInt(Math.round(pct * 100))) / 10000n;
            if (liquidityToRemove === 0n)
                liquidityToRemove = 1n; // prevent zero removal
            warnings.push(`Percentage mode: removing ${pct}% of position liquidity (${liquidityToRemove.toString()} of ${totalLiquidity.toString()}).`);
        }
        else {
            liquidityToRemove =
    ...
    if (!Array.isArray(args.ids) || !Array.isArray(args.amounts)) {
        throw new MantleMcpError("INVALID_INPUT", "Merchant Moe removeLiquidity requires 'ids' and 'amounts' arrays.", "Provide arrays of bin IDs and corresponding amounts to remove.", { provider });
    }
    const routerAddress = getContractAddress("merchant_moe", "lb_router_v2_2", network);
    const binStep = typeof args.bin_step === "number" ? args.bin_step : 20;
    const data = encodeFunctionData({
        abi: LB_ROUTER_ABI,
    ...
    return {
        ...
        human_summary: `Remove liquidity on Merchant Moe LB: ${tokenA.symbol}/${tokenB.symbol} (${args.ids.length} bins)`,
        unsigned_tx: {
            to: routerAddress,
            data,
            value: "0x0",
...
function buildV3RemoveLiquidity(params) {
    const { provider, tokenId, liquidity, amount0Min, amount1Min, recipient, deadline, network, now } = params;
    const positionManager = getContractAddress(provider, "position_manager", network);
...
// =========================================================================
export async function buildAaveSupply(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    ...
    const amountRaw = requirePositiveAmount(args.amount, "amount", asset.decimals);
    const onBehalfOf = requireAddress(args.on_behalf_of ?? args.recipient, "on_behalf_of");
    ...
    if (reserve.isolationMode) {
        ...
        warnings.push(`ISOLATION MODE: ${reserve.symbol} is an Isolation Mode asset (debt ceiling $${ceilingUsd}). ` +
...
// =========================================================================
export async function buildAaveBorrow(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const assetInput = requireString(args.asset, "asset");
    const reserve = requireAaveReserve(assetInput);
    ...
    const interestRateMode = typeof args.interest_rate_mode === "number"
    ...
    if (!reserve.borrowableInIsolation) {
        try {
            ...
            const [isoBalances, nonIsoBalances] = await Promise.all([
                Promise.all(isolationReserves.map(r => client.readContract({
                    address: r.aToken,
                    abi: ERC20_ABI,
                    functionName: "balanceOf",
                    args: [onBehalfOf]
                }).catch(() => 0n))),
                Promise.all(nonIsolationReserves.map(r => client.readContract({
                    address: r.aToken,
                    abi: ERC20_ABI,
                    functionName: "balanceOf",
                    args: [onBehalfOf]
            ...
            if (hasIsolationCollateral && !hasNonIsolationCollateral) {
                ...
                const isoAssets = isolationReserves
                    ...
                    .join(", ");
                const borrowable = isolationBorrowableSymbols().join(", ");
                throw new MantleMcpError("ISOLATION_MODE_BORROW_BLOCKED", `Cannot borrow ${reserve.symbol}: borrower (${onBehalfOf}) is in Isolation Mode ` +
                    `with only ${isoAssets} as collateral. ${reserve.symbol} is not borrowable in Isolation Mode.`, `In Isolation Mode you can only borrow: ${borrowable}. ` +
                    `To borrow ${reserve.symbol}, supply additional non-isolation collateral (e.g. USDC).`, { borrower: onBehalfOf, collateral: isoAssets, asset: reserve.symbol });
            }
        }
        catch (e) {
            // Re-throw our own errors; swallow RPC failures as a non-blocking warning
            if (e instanceof MantleMcpError)
                throw e;
            warnings.push(`ISOLATION MODE WARNING: ${reserve.symbol} is NOT borrowable in Isolation Mode. ` +
...
// =========================================================================
export async function buildAaveRepay(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const assetInput = requireString(args.asset, "asset");
    const reserve = requireAaveReserve(assetInput);
    ...
    const amountRaw = args.amount === "max"
        ? MAX_UINT256
...
// =========================================================================
export async function buildAaveWithdraw(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const assetInput = requireString(args.asset, "asset");
    const reserve = requireAaveReserve(assetInput);
        ...
        return {
            ...
            pairs: pairs.map((p) => ({
                ...
                ...(p.provider === "merchant_moe"
                    ? { bin_step: p.binStep, version: p.version }
                    : { fee_tier: p.feeTier })
            })),
            count: pairs.length
        };
...
export async function buildCollectFees(args, deps) {
    const d = withDeps(deps);
    const { network } = normalizeNetwork(args);
    // Validate provider
    const providerInput = requireString(args.provider, "provider").toLowerCase();
    if (providerInput !== "agni" && providerInput !== "fluxion") {
        throw new MantleMcpError("INVALID_INPUT", `provider must be 'agni' or 'fluxion' for V3 fee collection.`, "Use provider='agni' or provider='fluxion'.", { provider: providerInput });
    }
    ...
    try {
        ...
        throw new MantleMcpError("INVALID_INPUT", `token_id must be a valid integer.`, "Provide the NFT token ID as a string (e.g. '12345').", { token_id: tokenIdStr });
    }
    if (tokenId < 0n) {
        throw new MantleMcpError("INVALID_INPUT", `token_id must be non-negative.`, "Provide a valid NFT token ID.", { token_id: tokenIdStr });
    }
    // Validate recipient
    const recipient = requireAddress(args.recipient, "recipient");
    const positionManager = getContractAddress(provider, "position_manager", network);
...
export const defiWriteTools = {
    mantle_buildApprove: {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                spender: {
                    type: "string",
                    description: "Address of the contract to approve (must be on the whitelist)."
                },
                amount: {
                    type: "string",
                    description: "Decimal amount to approve (e.g. '100'). Use 'max' for unlimited."
                },
                owner: {
                    type: "string",
                    description: "Wallet address that owns the tokens. Used to check existing allowance and skip if sufficient."
                },
                network: {
                    type: "string",
    ...
    mantle_buildWrapMnt: {
        ...
        description: "Build an unsigned transaction to wrap MNT into WMNT. Returns calldata with value field set to the wrap amount.\n\nExamples:\n- Wrap 10 MNT: amount='10'\n- Wrap 0.5 MNT: amount='0.5'",
        inputSchema: {
            type: "object",
            properties: {
                amount: {
    ...
    mantle_buildUnwrapMnt: {
        ...
        description: "Build an unsigned transaction to unwrap WMNT back to MNT.\n\nExamples:\n- Unwrap 10 WMNT: amount='10'\n- Unwrap 0.5 WMNT: amount='0.5'",
        inputSchema: {
            type: "object",
            properties: {
                amount: {
    ...
    mantle_buildSwap: {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                fee_tier: {
                    type: "number",
                    description: "V3 fee tier (500=0.05%, 3000=0.3%, 10000=1%). Auto-resolved from known pairs for agni/fluxion."
                },
                bin_step: {
                    type: "number",
                    description: "LB bin step (1=stablecoins, 5=LSTs, 20=volatile). Auto-resolved from known pairs for merchant_moe."
                },
                network: {
                    type: "string",
                    description: "Network: 'mainnet' (default) or 'sepolia'."
    ...
    mantle_buildAddLiquidity: {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                amount_usd: {
                    type: "number",
                    description: "USD amount to invest (auto-splits 50/50 between tokens using live prices). " +
    ...
    mantle_buildRemoveLiquidity: {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                percentage: {
                    type: "number",
                    description: "Percentage of position liquidity to remove (1-100). For agni/fluxion. " +
    ...
    mantle_buildAaveSupply: {
        ...
        description: "Build an unsigned Aave V3 supply (deposit) transaction. Remember to approve the asset for the Pool contract first.\n\n" +
    ...
    mantle_buildAaveBorrow: {
        ...
        description: "Build an unsigned Aave V3 borrow transaction. Requires sufficient collateral deposited first.\n\n" +
    ...
    mantle_buildAaveRepay: {
        ...
        description: "Build an unsigned Aave V3 repay transaction. Use amount='max' to repay the full debt. Remember to approve the asset for the Pool first.\n\nExamples:\n- Repay 50 USDC: asset='USDC', amount='50', on_behalf_of='0x...'\n- Repay full WMNT debt: asset='WMNT', amount='max', on_behalf_of='0x...'",
        inputSchema: {
            type: "object",
            properties: {
                asset: {
    ...
    mantle_buildAaveWithdraw: {
        ...
        description: "Build an unsigned Aave V3 withdraw transaction. Use amount='max' to withdraw entire balance. May lower health factor.\n\nExamples:\n- Withdraw 50 USDC: asset='USDC', amount='50', to='0x...'\n- Withdraw all WMNT: asset='WMNT', amount='max', to='0x...'",
        inputSchema: {
            type: "object",
            properties: {
                asset: {
    ...
    mantle_getSwapPairs: {
        ...
        description: "List known trading pairs and their pool parameters for a DEX. Returns bin_step (Merchant Moe) or fee_tier (Agni/Fluxion) for each pair. Call this BEFORE mantle_buildSwap to get the correct pool parameters.\n\nExamples:\n- All Merchant Moe pairs: provider='merchant_moe'\n- All Agni pairs: provider='agni'\n- All pairs across all DEXes: (no provider)",
        inputSchema: {
            type: "object",
            properties: {
                provider: {
                    type: "string",
                    description: "Filter by DEX: 'agni', 'fluxion', or 'merchant_moe'. Omit for all DEXes."
                }
            },
            required: []
        },
