...
const ALLOWED_PROBE_METHODS = ["eth_chainId", "eth_blockNumber", "eth_getBalance"];
const defaultDeps = {
...
export async function checkRpcHealth(args, deps) {
...
export async function probeEndpoint(args, deps) {
    const resolvedDeps = withDeps(deps);
    const endpointInput = typeof args.rpc_url === "string" ? args.rpc_url : "";
    if (!endpointInput) {
        throw new MantleMcpError("INVALID_INPUT", "rpc_url is required.", "Provide rpc_url for mantle_probeEndpoint.", null);
    }
    const endpoint = (await ensureEndpointSafe(endpointInput)).toString();
    const methodInput = typeof args.method === "string" ? args.method : "eth_blockNumber";
    if (!ALLOWED_PROBE_METHODS.includes(methodInput)) {
        throw new MantleMcpError("INVALID_INPUT", `Method not allowed: ${methodInput}`, `Use one of: ${ALLOWED_PROBE_METHODS.join(", ")}.`, { method: methodInput });
    }
    ...
    try {
        ...
        throw new MantleMcpError("ENDPOINT_UNREACHABLE", error instanceof Error ? error.message : String(error), "Verify endpoint URL and method parameters, then retry.", { endpoint, method, retryable: true });
    }
}
export const diagnosticsTools = {
    checkRpcHealth: {
        name: "mantle_checkRpcHealth",
        description: "Check RPC endpoint health and chain-id consistency. Examples: mainnet https://rpc.mantle.xyz should match chain 5000 and support USDC workflows at 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    probeEndpoint: {
        ...
        description: "Probe a specific JSON-RPC endpoint with a minimal method call. Examples: eth_blockNumber against https://rpc.mantle.xyz for router checks near 0x319B69888b0d11cEC22caA5034e25FfFBDc88421.",
        inputSchema: {
            type: "object",
            properties: {
