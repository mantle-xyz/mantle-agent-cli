...
import { fetchTokenListSnapshot } from "../lib/token-list.js";
import { findTokenBySymbol, resolveTokenInput as resolveTokenFromQuickRef } from "../lib/token-registry.js";
...
async function fetchDefiLlamaTokenPrices(network, tokenAddresses) {
    ...
    const record = payload && typeof payload === "object" && payload.coins && typeof payload.coins === "object"
        ? payload.coins
        : {};
    const out = {};
...
function findTokenInCanonical(snapshot, symbol, chainId) {
    const match = snapshot.tokens.find((token) => token.chainId === chainId && token.symbol.toLowerCase() === symbol.toLowerCase());
    if (!match) {
        return null;
    }
    return { address: match.address, decimals: match.decimals };
}
export async function getTokenInfo(args, deps) {
    ...
    return {
        ...
        total_supply_normalized: metadata.totalSupply != null && metadata.decimals != null
...
export async function getTokenPrices(args, deps) {
    const resolvedDeps = withDeps(deps);
    const { network } = normalizeNetwork(args);
    const baseCurrency = typeof args.base_currency === "string" && args.base_currency.toLowerCase() === "mnt"
        ? "mnt"
        : "usd";
    const tokens = Array.isArray(args.tokens) ? args.tokens.map(String) : [];
    if (tokens.length === 0) {
        throw new MantleMcpError("INVALID_INPUT", "At least one token is required.", "Provide one or more token symbols or addresses in `tokens`.", { field: "tokens" });
    }
    const resolvedInputs = await Promise.all(tokens.map(async (input) => {
        try {
    ...
    const dexPrices = await Promise.all(quotedAddresses.map((address) => fetchDexScreenerTokenPriceUsd(network, address)));
    const missing = [];
    ...
    const mntUsd = mntPricingAddress != null
    ...
    const prices = resolvedInputs.map((entry) => {
        ...
        if (baseCurrency === "mnt") {
            ...
            return {
                ...
                source,
                confidence: source === "dexscreener" ? "high" : "medium",
                quoted_at_utc: quotedAt,
                warnings: []
            };
        }
        return {
            ...
            price: usdPrice,
            source,
            confidence: usdPrice == null ? "low" : source === "dexscreener" ? "high" : "medium",
            quoted_at_utc: quotedAt,
    ...
    return {
        ...
        warnings: prices.some((entry) => entry.price === null)
            ? ["Prices are null when a trusted source is unavailable. Values are never fabricated."]
            : [],
        network
    };
}
export async function resolveToken(args, deps) {
    const resolvedDeps = withDeps(deps);
    const { network } = normalizeNetwork(args);
    ...
    if (!quickRef) {
        throw new MantleMcpError("TOKEN_NOT_FOUND", `Unknown token symbol: ${symbol}`, "Use mantle://registry/tokens to discover supported symbols.", { symbol, network });
    }
    const warnings = [];
    let tokenListChecked = false;
    let tokenListMatch = null;
    let tokenListAddress = null;
    let tokenListVersion = null;
    let source = "quick_ref";
    let confidence = "high";
        ...
        try {
            ...
            if (!canonical) {
                ...
                if (requireTokenListMatch) {
                    throw new MantleMcpError("TOKEN_REGISTRY_MISMATCH", `Token ${quickRef.symbol} missing from canonical token list.`, "Retry later or provide a token confirmed by the canonical token list.", { symbol: quickRef.symbol, network, token_list_version: tokenListVersion });
                }
                confidence = "low";
                ...
                if (!tokenListMatch) {
                    throw new MantleMcpError("TOKEN_REGISTRY_MISMATCH", `Token registry mismatch for ${quickRef.symbol}.`, "Stop execution and verify the token contract address from canonical sources.", {
            ...
            if (requireTokenListMatch) {
                throw new MantleMcpError("TOKEN_LIST_UNAVAILABLE", "Canonical token list is unavailable.", "Retry when token-list endpoint is reachable or configure a valid token-list URL.", { retryable: true, raw_error: error instanceof Error ? error.message : String(error) });
            }
...
export const tokenTools = {
    getTokenInfo: {
        ...
        description: "Read ERC-20 token metadata (name, symbol, decimals, total supply). Examples: token='USDC' -> 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9, token='WETH' -> 0xdEAddEaDdeadDEadDEADDEAddEADDEAddead1111.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    getTokenPrices: {
        ...
        description: "Read token prices for valuation workflows; returns null when no trusted source exists. Examples: tokens=['USDC','WMNT'] on mainnet (0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9, 0x78c1b0C915c4FAA5FffA6CAbf0219DA63d7f4cb8).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    resolveToken: {
        ...
        description: "Resolve token symbol using quick reference plus canonical token-list cross-check. Examples: symbol='USDC' -> 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9, symbol='mETH' -> 0xcDA86A272531e8640cD7F1a92c01839911B90bb0.",
        inputSchema: {
            type: "object",
            properties: {
