...
import { findRegistryByAddress, listRegistryEntries } from "../lib/registry.js";
function confidenceFromTimestamp(timestamp) {
    if (!timestamp) {
        return "low";
    }
...
function matchCategory(entry, category) {
    return category === "any" ? true : entry.category === category;
}
function findEntry(entries, identifier, category) {
    const needle = normalizeLookupText(identifier);
    const byKey = entries.find((entry) => matchCategory(entry, category) && normalizeLookupText(entry.key) === needle);
    if (byKey) {
        return byKey;
    }
    const byAlias = entries.find((entry) => matchCategory(entry, category) &&
        (entry.aliases ?? []).some((alias) => normalizeLookupText(alias) === needle));
    if (byAlias) {
        return byAlias;
    }
    const byLabel = entries.find((entry) => matchCategory(entry, category) && normalizeLookupText(entry.label) === needle);
    return byLabel ?? null;
}
const defaultValidateDeps = {
    getClient: getPublicClient
};
export async function resolveAddress(args) {
    ...
    const category = (typeof args.category === "string" ? args.category : "any");
    if (!identifier) {
        throw new MantleMcpError("ADDRESS_NOT_FOUND", "Identifier is required for address resolution.", "Pass identifier as a contract key, alias, or label.", null);
    }
    const entry = findEntry(listRegistryEntries(network), identifier, category);
    if (!entry) {
        throw new MantleMcpError("ADDRESS_NOT_FOUND", `No registry entry found for: ${identifier}`, "Use mantle://registry/contracts to discover valid keys and aliases.", { identifier, network, category });
    }
    return {
        identifier,
        network,
...
export async function validateAddress(args, deps = defaultValidateDeps) {
...
export const registryTools = {
    resolveAddress: {
        ...
        description: "Resolve trusted contract addresses by key, alias, or label from the Mantle registry. Examples: identifier='USDC' -> 0x09Bc4E0D864854c6aFB6eB9A9cdF58aC190D0dF9; identifier='Agni Router' -> 0x319B69888b0d11cEC22caA5034e25FfFBDc88421.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    validateAddress: {
        ...
        description: "Validate address format/checksum and optional bytecode presence. Examples: 0x78c1b0C915c4FAA5FffA6CAbf0219DA63d7f4cb8 (WMNT) and 0x319B69888b0d11cEC22caA5034e25FfFBDc88421 (Agni Router).",
        inputSchema: {
            type: "object",
            properties: {
