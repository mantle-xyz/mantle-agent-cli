...
// ---------------------------------------------------------------------------
const ALL_PAIRS = [...MOE_PAIRS, ...AGNI_PAIRS, ...FLUXION_PAIRS];
/**
 * Find a known trading pair by provider and token symbols.
 * Matches in either direction (A/B or B/A).
 */
export function findPair(provider, tokenA, tokenB, _network) {
    const a = tokenA.toLowerCase();
    const b = tokenB.toLowerCase();
    return (ALL_PAIRS.find((p) => p.provider === provider &&
        ((p.tokenA.toLowerCase() === a && p.tokenB.toLowerCase() === b) ||
            (p.tokenA.toLowerCase() === b && p.tokenB.toLowerCase() === a))) ?? null);
}
/**
 * Find a known trading pair by provider and token addresses.
 * Matches in either direction.
 */
export function findPairByAddress(provider, addressA, addressB, _network) {
