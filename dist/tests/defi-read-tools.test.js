...
import { getLendingMarkets, getPoolLiquidity, getPoolOpportunities, getProtocolTvl, getSwapQuote } from "../src/tools/defi-read.js";
describe("defi read tools", () => {
    ...
    it("falls back from dexscreener to subgraph for pool liquidity and records trace", async () => {
        ...
        expect(result.source_trace.map((item) => item.source)).toContain("dexscreener");
        expect(result.source_trace.map((item) => item.source)).toContain("subgraph");
    ...
    it("falls back from onchain to subgraph/indexer source for lending markets", async () => {
        ...
        expect(result.source_trace.map((item) => item.source)).toContain("onchain_aave");
        expect(result.source_trace.map((item) => item.source)).toContain("subgraph");
        expect(result.confidence?.score).toBeGreaterThan(0);
    });
    it("returns protocol tvl from DefiLlama for agni", async () => {
        ...
        expect(result.source_trace.map((item) => item.source)).toContain("defillama_protocol");
        expect(result.confidence?.score).toBeGreaterThan(0);
    });
    it("falls back to subgraph/indexer tvl source when DefiLlama is unavailable", async () => {
        ...
        expect(result.source_trace.map((item) => item.source)).toContain("indexer_sql");
        expect(result.partial).toBe(true);
    });
